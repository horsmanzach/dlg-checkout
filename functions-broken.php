...
if (isset($upfront_summary['installation'])) {
    ...
    // Add promotional pricing if exists
    if (isset($upfront_summary['installation'][2]) && isset($upfront_summary['installation'][3])) {
        $upfront_summary['installation']['Original Installation Price'] = $upfront_summary['installation'][2];
        $upfront_summary['installation']['Promotional Installation Price'] = $upfront_summary['installation'][3];
    }
...
function prepare_diallog_order_data($payment_response, $cardholder_name, $moneris_config) {
    ...
    // Add promotional installation pricing to internet plan if it exists
if (!empty($upfront_summary) && is_array($upfront_summary)) {
    // Find internet plan in cart to check for promotional installation pricing
    foreach (WC()->cart->get_cart() as $cart_item) {
        $product = $cart_item['data'];
        $product_cats = wp_get_post_terms($product->get_id(), 'product_cat', array('fields' => 'ids'));
        
        // Check if this is an internet plan (category 19)
        if (in_array(19, $product_cats)) {
            $internet_plan_id = $product->get_id();
            
            // Check for promotional installation price on this internet plan
            if (function_exists('get_field')) {
                $promo_install_price = get_field('dynamic_install_price', $internet_plan_id);
                if (empty($promo_install_price) && $promo_install_price !== '0') {
                    $promo_install_price = get_field('dynamic_install_price', 'product_' . $internet_plan_id);
                }
                $promo_install_price = is_numeric($promo_install_price) ? floatval($promo_install_price) : 0;
                
                // Add to upfront summary if promotional price exists
                if ($promo_install_price > 0) {
                    // Find the internet-plan in upfront_summary and add the field
                    foreach ($upfront_summary as $key => $item) {
                        if (is_array($item) && isset($item['category']) && $item['category'] === 'internet-plan') {
                            $upfront_summary[$key]['Dynamic Installation Sale Price'] = '$' . number_format($promo_install_price, 2);
                            error_log('Added Dynamic Installation Sale Price to internet plan: $' . number_format($promo_install_price, 2));
                            break;
                        }
                    }
                }
            }
            break; // Found internet plan, no need to continue loop
        }
    }
}
...
function upfront_fee_summary_shortcode() {
   ...
   $installation_product_obj = null; // NEW: Store the product object
   ...
   foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
       ...
       if ($is_installation) {
           ...
           $installation_product_obj = $product; // NEW: Store product object for later
...
if ($installation_found) {
    // Check for promotional installation pricing from internet plan's ACF field
    $promo_install_price = 0;
    $internet_plan_id = null;
    
    // Find the internet plan in cart (category 19)
    foreach ($cart->get_cart() as $check_item) {
        $check_product = $check_item['data'];
        $check_cats = wp_get_post_terms($check_product->get_id(), 'product_cat', array('fields' => 'ids'));
        if (in_array(19, $check_cats)) { // Category 19 is internet-plan
            $internet_plan_id = $check_product->get_id();
            break;
        }
    }
    
    // Get promotional price if internet plan has one set
    if ($internet_plan_id && function_exists('get_field')) {
        $promo_install_price = get_field('dynamic_install_price', $internet_plan_id);
        if (empty($promo_install_price) && $promo_install_price !== '0') {
            $promo_install_price = get_field('dynamic_install_price', 'product_' . $internet_plan_id);
        }
    ...
    // Use promotional price if available for subtotal calculation
    $final_install_price = ($promo_install_price > 0) ? $promo_install_price : $installation_price;
    ...
    // Format price with promo styling if applicable
    if ($promo_install_price > 0 && $promo_install_price != $installation_price) {
        $output .= '<td><span style="text-decoration: line-through;">' . wc_price($installation_price) . '</span><br><span style="color: green; font-weight: bold;">' . wc_price($promo_install_price) . '</span></td>';
    } else {
    ...
    $subtotal += $final_install_price;  // Use promo price in subtotal if available
}
...
function monthly_fee_summary_shortcode() {
    ...
    if (is_product() && $post && $current_product_id && $internet_plan_in_cart) {
        ...
        // Build product name display with promo blurb if available
    ...
    // Now loop through cart items and display their monthly fees
    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        $product = $cart_item['data'];
        $product_id = $product->get_id();
        $parent_id = $product->get_parent_id();
        ...
        // Check if this is an installation product - SKIP if it is
        $is_installation = in_array($installation_category_id, $product_cats) || $parent_id == $installation_product_id;
        
        if ($is_installation) {
            continue; // Skip installation products
        }
        
        // Check if this is a deposit product - SKIP if it is
        $is_deposit = $deposit_category_id && in_array($deposit_category_id, $product_cats);
        
        if ($is_deposit) {
            continue; // Skip deposit products
        }
        
        // Check if this is the Pay After Service deposit product - SKIP if it is
        if ($product_id == 265827) {
            continue; // Skip Pay After Service deposit
        }
        ...
        // Get terms from parent product if this is a variation
        if ($parent_id > 0) {
            $terms = get_the_terms($parent_id, 'product_cat');
        } else {
            $terms = get_the_terms($product_id, 'product_cat');
...
function upfront_fee_total_shortcode($atts) {
    error_log('=== UPFRONT_FEE_TOTAL_SHORTCODE CALLED ===');
    
    // Use get_upfront_fee_summary() for consistent calculations
    if (function_exists('get_upfront_fee_summary')) {
        error_log('get_upfront_fee_summary function EXISTS - calling it now');
        $summary = get_upfront_fee_summary();
        error_log('Summary returned: ' . print_r($summary, true));
        error_log('Summary grand_total value: ' . $summary['grand_total'][1]);
        $total = $summary['grand_total'][1];
        $total_display = wc_price($total);
        error_log('Final total_display: ' . strip_tags($total_display));
    } else {
        error_log('get_upfront_fee_summary function NOT FOUND');
        $total_display = '$0.00';
    }
    
    error_log('=== END UPFRONT_FEE_TOTAL_SHORTCODE ===');
...
if ($product_id == 265084 || $parent_id == 265084) {
    ...
    if ($product_price > 0) {
        $item_data = array(
            ...
            'dates' => $installation_dates
        );
        
        // Add promo info if available
        if ($promo_install_price > 0 && $promo_install_price != $product_price) {
            $item_data['original_price'] = $product_price;
            $item_data['promo_price'] = $promo_install_price;
        }
...
function get_upfront_fee_summary() {
    ...
    error_log("=== NEW VERSION 2.0 - UPDATED INSTALLATION CODE ===");
    ...
    foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
        ...
        // Check which summary category this product belongs to
        $product_category = null;
        
        // Check ALL product categories to find a match with summary categories
        foreach ($product_cat_ids as $cat_id) {
            $product_cat = get_term($cat_id, 'product_cat');
            ...
            // Handle 'modems-new' category - treat it as 'modems'
            if ($cat_slug == 'modems-new') {
                $cat_slug = 'modems';
            }
            
            // Check if this category exists in our summary array
            if (array_key_exists($cat_slug, $summary)) {
                $product_category = $cat_slug;
                error_log("Matched product to summary category: $product_category");
                break;
            }
        }
        
        // If no matching category found, skip this product
        if ($product_category === null) {
            error_log("Product " . $_product->get_name() . " - no matching summary category found");
            continue;
        }
        ...
        if ( $product_category == "modems" && $final_deposit > 0 ) {
            ...
            // SPECIAL CASE: Handle installation with promotional pricing FIRST
            if ($product_category == 'installation') {
                $product_price = $_product->get_price();
                ...
                error_log('=== LOOKING FOR INTERNET PLAN ===');
                
                // Find the internet plan in cart (category 19)
                foreach (WC()->cart->get_cart() as $check_item) {
                    $check_product = $check_item['data'];
                    $check_cats = wp_get_post_terms($check_product->get_id(), 'product_cat', array('fields' => 'ids'));
                    error_log('Checking product ' . $check_product->get_id() . ' - categories: ' . print_r($check_cats, true));
                    if (in_array(19, $check_cats)) { // Category 19 is internet-plan
                        $internet_plan_id = $check_product->get_id();
                        error_log('FOUND internet plan ID: ' . $internet_plan_id);
                        break;
                    }
                }
                
                error_log('Internet plan ID after search: ' . ($internet_plan_id ? $internet_plan_id : 'NULL'));
                
                // Get promotional price if internet plan has one set
                if ($internet_plan_id && function_exists('get_field')) {
                    $promo_install_price = get_field('dynamic_install_price', $internet_plan_id);
                    error_log('ACF get_field direct: ' . ($promo_install_price !== false ? $promo_install_price : 'FALSE'));
                    
                    if (empty($promo_install_price) && $promo_install_price !== '0') {
                        $promo_install_price = get_field('dynamic_install_price', 'product_' . $internet_plan_id);
                        error_log('ACF get_field with product_ prefix: ' . ($promo_install_price !== false ? $promo_install_price : 'FALSE'));
                    }
                    $promo_install_price = is_numeric($promo_install_price) ? floatval($promo_install_price) : 0;
                    error_log('Final promo_install_price after conversion: ' . $promo_install_price);
                }
                
                // Use promotional price if available, otherwise use product price
                $final_install_price = ($promo_install_price > 0) ? $promo_install_price : $product_price;
                
                error_log("Installation pricing - Product: $$product_price, Promo: $$promo_install_price, Final: $$final_install_price");
                
                $summary[$product_category][0] = $_product->get_title()." ".( $show_included_taxes ?"(inc taxes)":"")."" ;
                $summary[$product_category][1] = round(floatval($final_install_price), 2);
                
                // Add promotional pricing info to array for thank-you page and email display
                if ($promo_install_price > 0 && $promo_install_price != $product_price) {
                    $summary[$product_category][2] = round(floatval($product_price), 2);  // original_price
                    $summary[$product_category][3] = round(floatval($promo_install_price), 2);  // promo_price
                }
                
                $summary['taxes'][1] += round( floatval ( ($summary[$product_category][1] * $tax_rate ) / 100 ) , 2 );
                error_log("Installation product: " . $summary[$product_category][0] . " = $" . $summary[$product_category][1]);
            }
            // SPECIAL CASE: For deposit category products
            else if ($product_category == 'deposit') {
                ...
                // Handle all other regular products
                $product_price = $_product->get_price();
                $summary[$product_category][0] = $_product->get_title()." ".( $show_included_taxes ?"(inc taxes)":"")."" ;
                $summary[$product_category][1] = round(floatval($product_price), 2);
                $summary['taxes'][1] += round( floatval ( ($summary[$product_category][1] * $tax_rate ) / 100 ) , 2 );
